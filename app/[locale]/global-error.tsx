...
export default function GlobalErrorPage({ error, reset }: GlobalErrorPageProps) {
  ...
  const message =
    error.message?.trim()
    || (locale === "en" ? "A critical error occurred." : "Сталася критична помилка.");
      ...
      <body style={bodyStyle}>
        <div style={shellStyle}>
          <div style={cardStyle} role="alert">
            <h2>{locale === "en" ? "This page is temporarily unavailable" : "Сторінку тимчасово недоступно"}</h2>
            <p>{message}</p>
            <button type="button" style={buttonStyle} onClick={reset}>
              {locale === "en" ? "Reload" : "Перезавантажити"}
            </button>
          </div>
        </div>
      </body>
